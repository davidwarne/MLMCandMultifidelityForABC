function [E,V,F]= ABCMLMC(N,M,p,supp0,s,rho,epsilon,...
                          s_approx,rho_apporx,epsilon_approx,tau,f,h)
...
%   Thomas ). Prescott[4] (prescott@maths.ox.ac.uk)
%   
% Affiliations:
...
for l=1:L
    ...
    pl = @() p(supp.l,supp.u);
    supp_prev = supp;
    [El,Vl,~,~,~,~,~,theta{l},weights{l}] = ABCAdaptiveGradientMultifidelity(N(l),M(l),pl,s,rho,epsilon(l),s_approx,rho_approx,epsilon_approx(l),f)
